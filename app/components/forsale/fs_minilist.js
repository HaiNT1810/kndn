  ...
  return (
    <View>
      <List renderHeader={<Component_Header
        headerText={headerText || "Thông tin chào bán"}
        seeMore={{ show: showSeeMore == null ? true : showSeeMore, onPressFunc: onPressSeeMore }}
        textIcon={{ name: "badge-dollar", color: Colors.blueHope, size: 18 }}></Component_Header>}>
        {
          data.map((x, index) => {
            return (
              <Item key={"fb" + index} arrow="empty" multipleLine wrap={true} onPress={() => {
                navigation.navigate('ForSale_DetailScreen', { id: x.ID });
              }} thumb={
                <Image
                  source={x.image}
                ...
                <Text>
                  ...
                  &emsp;
                  {x.tags?.map(a => { return (<Tag style={{ paddingRight: 3 }} key={"fsmt" + a} small>{a}</Tag>) })}
                </Text>
              </Item>
            )
          })
