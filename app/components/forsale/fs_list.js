...
const RenderItem = (props) => {
  ...
  return (
    <View style={{ width: '100%' }}>
      ...
      {!showDetail ? (
        <Accordion key={item.ID} onChange={onChange} activeSections={state.activeSections}>
          <Accordion.Panel header={
            <View style={{ width: '98%' }}>
              ...
              <Text style={{ marginBottom: 10 }}>
                ...
                &emsp;
                {item.tags?.map(a => { return (<Tag style={{ paddingRight: 3 }} key={"fst" + item.ID + a} small>{a}</Tag>) })}
              </Text>
            </View>
          }>
            {state.activeSections.length != 0 ?
              <View style={{ width: '100%' }}>
                ...
                />
                <Button type="primary" onPress={() => { navigation.navigate("ForSale_DetailScreen", { id: item.ID }) }}>Xem thông tin</Button>
              </View> : <></>
            }
          </Accordion.Panel >
        </Accordion >)
